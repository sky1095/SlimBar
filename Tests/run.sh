...
Path('build/menu-test/main.swift').write_text(source + Path('Tests/menu-refresh.swift').read_text() + Path('Tests/features.swift').read_text() + Path('Tests/status-presentation.swift').read_text() + Path('Tests/android.swift').read_text())
PY
source ./sparkle.sh
xcrun swiftc -swift-version 5 -module-cache-path "$PWD/build/module-cache" build/menu-test/main.swift Sources/Features.swift Sources/Android.swift Sources/StatusPresentation.swift Sources/Updates.swift -o build/menu-test/check \
